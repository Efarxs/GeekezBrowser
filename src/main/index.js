...
// A pleasant, well-saturated random color for the per-launch window frame
// (Chrome --install-autogenerated-theme) + matching profile-card border.
// Randomized each launch; returns { r, g, b, hex }. Uses HSL with a random
// hue and fixed S/L so every color is vivid but not washed out or too dark.
...
async function handleApiRequest(method, pathname, body, params, context = {}) {
    ...
    if (method === 'PATCH' && pathname === '/api/settings') {
        ...
        const WRITABLE = {
            ...
            enableInstanceColor:   v => typeof v === 'boolean',
...
function broadcastProfileStatus(profileId, status, color = null) {
    const windows = BrowserWindow.getAllWindows();
    for (const win of windows) {
        try {
            if (!win || win.isDestroyed() || !win.webContents || win.webContents.isDestroyed()) continue;
            win.webContents.send('profile-status', { id: profileId, status, color });
            if (status === 'stopped') {
...
async function cleanupProfileRuntime(profileId, options = {}) {
    ...
    if (detachTunnel) {
        ...
        // Clear the userStopRequested marker ONLY when there's no live browser
        // left to kill here. If we're about to forceKill a live browserPid, the
        // browser's 'exit' handler fires asynchronously AFTER this function and
        // must still see the marker to classify the exit as user-initiated
        // (otherwise it fires a bogus "浏览器异常退出" crash toast). The exit
        // handler consumes/deletes the marker itself. When the browser was
        // already dead (no browserPid), the exit handler won't fire, so we must
        // clear the marker here to avoid a stale entry suppressing a later crash.
...
ipcMain.handle('get-profile-runtime-state', () => ({
    ...
    // Per-launch frame colors so the UI can restore card borders on refresh.
    frameColors: Object.fromEntries(
        Object.entries(activeProcesses)
            .filter(([, v]) => v && v.frameColor)
            .map(([id, v]) => [id, v.frameColor])
    )
}));
...
const launchProfileHandler = async (event, profileId, preferredLang, launchOptions = {}) => {
    ...
    let launchFrameColor = null;   // per-launch window frame color (watermark-off only)
    ...
    try {
        ...
        // Per-launch random window frame color. Tints the browser chrome
        // (frame / toolbar) via --install-autogenerated-theme — page JS can't
        // read it, so zero fingerprint surface — and the same hex is broadcast
        // to tint the profile card border, so window↔card pair up at a glance
        // (essential once the page-level watermark is off). Toggle in Settings
        // (enableInstanceColor, default on); randomized each launch, not
        // persisted. When off, Chrome uses its default theme.
        if (settings.enableInstanceColor !== false) {
            launchFrameColor = randomFrameColor();
            launchArgs.push(`--install-autogenerated-theme=${launchFrameColor.r},${launchFrameColor.g},${launchFrameColor.b}`);
        }
        ...
        activeProcesses[profileId] = {
            ...
            frameColor: launchFrameColor ? launchFrameColor.hex : null
        };
        ...
        sender.send('profile-status', { id: profileId, status: 'running', color: launchFrameColor ? launchFrameColor.hex : null });
        // Also broadcast to all renderer windows (the `sender` above is the
        // HTTP response for API-initiated launches, not the UI) so the profile
        // card border picks up the frame color regardless of launch source.
        broadcastProfileStatus(profileId, 'running', launchFrameColor ? launchFrameColor.hex : null);
        refreshTrayMenu().catch(() => { });
