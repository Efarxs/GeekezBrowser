...
const launchProfileHandler = async (event, profileId, preferredLang, launchOptions = {}) => {
    ...
    try {
        ...
        // Per-launch random window frame color. When the page-level watermark
        // is off, this is how the user tells otherwise-identical instances
        // apart: --install-autogenerated-theme tints the browser chrome (frame
        // / toolbar), which page JS can't read — so it adds zero fingerprint
        // surface. The same hex is broadcast to the renderer so the profile
        // card border matches. Randomized every launch; not persisted.
        if (!enableWatermark) {
